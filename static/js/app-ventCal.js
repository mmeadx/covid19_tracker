console.log("app-ventCal.js running")
  ...
  d3.json("https://api.covidtracking.com/v1/us/daily.json").then((data) => {
    ...
    const dateValues = data.map(dv => ({
        ...
        value: Number(dv.onVentilatorCurrently)
    }));

    const svg = d3.select("#svgVent");
    const { width, height } = document
        .getElementById("svgVent")
    ...
    function draw() {
        ...
        return {
            upperBound,
            lowerBound,
            color: d3.interpolateBuPu(upperBound / maxValue),
            selected: true
        };
        });
